...
std::pair<int,int> test_LinearSystems()
{
   ...
   ( TestCholeskyDecomposition()    ? ++nsucc : ++nfail );
   ( TestCholeskySolve()            ? ++nsucc : ++nfail );
   ( TestRSPDInv()                  ? ++nsucc : ++nfail );
   ( TestLeastSquaresSolve()        ? ++nsucc : ++nfail );
   ( TestAffineTransformation()     ? ++nsucc : ++nfail );
